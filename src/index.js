...
function getCurrencyCode( cartItem ) {
	return cartItem?.prices?.currency_code || currencyCode;
}
...
function buildSecondaryText( totals ) {
	...
	if ( code !== 'BGN' && code !== 'EUR' ) return null;
	return code === 'BGN'
		? `/ ${ fmt( minorUnitsToFloat( raw, minorUnit ) / EUR_RATE ) } €`
		: `/ ${ fmt( minorUnitsToFloat( raw, minorUnit ) * EUR_RATE ) } лв.`;
}
